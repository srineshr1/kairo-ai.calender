...
export default function WhatsAppToast({ count, message, onDismiss }) {
    ...
    <div className="fixed bottom-4 right-4 z-50 animate-slideUp">
      <div className="bg-white dark:bg-[#1f1d30] rounded-xl shadow-2xl border border-black/10 dark:border-white/10 overflow-hidden max-w-sm">
        ...
        <div className="px-4 py-3">
          {message ? (
            <div>
              <p className="text-sm font-medium text-gray-800 dark:text-gray-200">{message.from}</p>
              <p className="text-sm text-gray-600 dark:text-gray-400 mt-1 line-clamp-2">{message.body}</p>
            </div>
          ) : (
            <p className="text-sm text-gray-600 dark:text-gray-400">
              {displayCount} new message{displayCount > 1 ? 's' : ''} received
            </p>
          )}
        </div>
