...
export default function DayColumn({ date, events, searchQuery, onEventClick, onSlotClick, awakeStart, awakeEnd }) {
  ...
  return (
    <div
      ref={setNodeRef}
      className={`border-l border-black/[0.06] relative ${isOver ? 'bg-accent/5' : ''} transition-colors`}
    ...
    >
      ...
      {sleepTopZoneH > 0 && (
        ...
        >
          <div className="w-full h-full bg-black/[0.035]" />
          {/* Faint border at awakeStart */}
          <div className="absolute bottom-0 left-0 right-0 border-b-2 border-dashed border-black/[0.12]" />
        </div>
      )}
      ...
      {sleepBottomZoneH > 0 && (
        ...
        >
          ...
          <div className="absolute top-0 left-0 right-0 border-t-2 border-dashed border-black/[0.12]" />
          <div className="w-full h-full bg-black/[0.035]" />
        </div>
      )}
      ...
      {showNow && (
        ...
        >
          <div className="w-2.5 h-2.5 rounded-full bg-gray-800 -ml-1.5 flex-shrink-0" />
          <div className="flex-1 h-[1.5px] bg-gray-800" />
        </div>
      )}
