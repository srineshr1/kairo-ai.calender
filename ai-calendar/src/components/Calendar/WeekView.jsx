...
export default function WeekView({ onEventClick, onSlotClick }) {
  ...
  return (
    <div className="flex flex-col flex-1 overflow-hidden">
      {/* Date range nav */}
      <div className="bg-white px-7 py-3 flex items-center gap-3 border-b border-black/[0.06] flex-shrink-0">
        <button
        ...
        <h2 className="font-display text-[22px] text-gray-800 tracking-tight min-w-[200px]">
          {rangeLabel}
        </h2>
        <button
      ...
      {/* Timeline */}
      <div className="flex-1 overflow-y-auto light-scroll overflow-x-hidden">
        <DndContext
          sensors={sensors}
        ...
        >
          <div key={animKey} className={slideClass} style={{ willChange: 'transform, opacity' }}>
            {/* Week header */}
            <div
              className="grid sticky top-0 z-20 bg-white border-b border-black/[0.06]"
                ...
                return (
                  <div key={fmtDate(d)} className="py-3 text-center">
                    <span className={`
                      ...
                      ${today
                        ? 'bg-sidebar-deep text-white w-10 h-10 rounded-full inline-flex items-center justify-center text-[18px]'
                        : 'text-gray-300 block'}
                    `}>
                      {d.getDate()}
                    </span>
                    <span className="text-[11px] uppercase tracking-wide text-gray-400 mt-1 block">
                      {format(d, 'EEE')}
                    </span>
                  </div>
                )
                ...
                {HOURS.map((h) => (
                  <div key={h} className="relative" style={{ height: PX_PER_HOUR }}>
                    <span className={`absolute -top-2 right-2 text-[11px] tabular-nums ${
                      h >= awakeStart && h < awakeEnd ? 'text-gray-400' : 'text-gray-300'
                    }`}>
